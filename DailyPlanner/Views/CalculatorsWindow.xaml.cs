...
namespace DailyPlanner.Views
{
    public partial class CalculatorsWindow : Window
    {
        ...
        {
            InitializeComponent();
            this.Language = XmlLanguage.GetLanguage(DailyPlanner.Services.LanguageService.CurrentCulture.IetfLanguageTag);
        }
        ...
        {
            MessageBox.Show(msg, GetLoc("Msg_Error"), MessageBoxButton.OK, MessageBoxImage.Error);
        }
        ...
        private void CalcRIR_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                RirResultText.Text = string.Format(GetLoc("RIR_Result"), oneRM);
                
                string details = string.Format(GetLoc("RIR_Detail_Format"), w, reps, rir);
                details += "\n" + string.Format(GetLoc("RIR_Level_95"), 0.95 * oneRM);
                details += "\n" + string.Format(GetLoc("RIR_Level_90"), 0.90 * oneRM);
                details += "\n" + string.Format(GetLoc("RIR_Level_85"), 0.85 * oneRM);
                details += "\n" + string.Format(GetLoc("RIR_Level_80"), 0.80 * oneRM);
                details += "\n" + string.Format(GetLoc("RIR_Level_75"), 0.75 * oneRM);
                details += "\n" + string.Format(GetLoc("RIR_Level_70"), 0.70 * oneRM);
                details += "\n" + string.Format(GetLoc("RIR_Level_65"), 0.65 * oneRM);
                
                RirDetailText.Text = details;
                RirResult.Visibility = Visibility.Visible;
            }
            catch { ShowError(GetLoc("Msg_InvalidInput")); }
        }
        ...
        private void CalcBodyFat_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                if (_bfIsMale)
                {
                    category = bf < 6  ? GetLoc("BF_Category_Essential") : bf < 14 ? GetLoc("BF_Category_Athlete") : bf < 18 ? GetLoc("BF_Category_Fit") : bf < 25 ? GetLoc("BF_Category_Acceptable") : GetLoc("BF_Category_Obese");
                    ideal = GetLoc("BF_Ideal_Men");
                }
                else
                {
                    category = bf < 14 ? GetLoc("BF_Category_Essential") : bf < 21 ? GetLoc("BF_Category_Athlete") : bf < 25 ? GetLoc("BF_Category_Fit") : bf < 32 ? GetLoc("BF_Category_Acceptable") : GetLoc("BF_Category_Obese");
                    ideal = GetLoc("BF_Ideal_Women");
                }

                BfResultText.Text  = string.Format(GetLoc("BF_Result"), bf);
                BfCategoryText.Text = $"{category}\n{ideal}\n\n{string.Format(GetLoc("BF_LeanMass_Format"), 100 - bf)}";
        ...
        private void CalcBMR_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                BmrResultText.Text = string.Format(GetLoc("Calc_TDEE_Total"), tdee);
                BmrDetailText.Text = string.Format(GetLoc("Calc_BMR_Format"), bmr) + "\n\n" +
                                     GetLoc("Calc_Daily_Goals") + "\n" +
                                     string.Format(GetLoc("Calc_Goal_Lose"), tdee - 500) + "\n" +
                                     string.Format(GetLoc("Calc_Goal_Maintain"), tdee) + "\n" +
                                     string.Format(GetLoc("Calc_Goal_Gain"), tdee + 300);
        ...
        private void CalcMacro_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                string goalLabel = goal == 0 ? GetLoc("Macro_Goal_Lose") : goal == 2 ? GetLoc("Macro_Goal_Gain") : GetLoc("Macro_Goal_Maintain");

                MacroResultText.Text = $"{goalLabel} — {totalCal:F0} kcal";

                MacroDetailText.Text = string.Format(GetLoc("Macro_Detail_Protein"), proteinG, proteinCal, proteinCal / totalCal * 100) + "\n" +
                                     string.Format(GetLoc("Macro_Detail_Fat"), fatG, fatCal, fatRatio * 100) + "\n" +
                                     string.Format(GetLoc("Macro_Detail_Carbs"), carbG, carbCal, carbCal / totalCal * 100) + "\n\n" +
                                     string.Format(GetLoc("Macro_Note"), proteinPerKg);
        ...
        private void CalcBMI_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                if (bmi < 18.5)
                {
                    category  = GetLoc("BMI_Category_Underweight");
                    detail    = GetLoc("BMI_Detail_Underweight");
                    indicator = GetLoc("BMI_Indicator_Underweight");
                }
                else if (bmi < 25.0)
                {
                    category  = GetLoc("BMI_Category_Normal");
                    detail    = GetLoc("BMI_Detail_Normal");
                    indicator = GetLoc("BMI_Indicator_Normal");
                }
                else if (bmi < 30.0)
                {
                    category  = GetLoc("BMI_Category_Overweight");
                    detail    = GetLoc("BMI_Detail_Overweight");
                    indicator = GetLoc("BMI_Indicator_Overweight");
                }
                else if (bmi < 35.0)
                {
                    category  = GetLoc("BMI_Category_Obese1");
                    detail    = GetLoc("BMI_Detail_Obese1");
                    indicator = GetLoc("BMI_Indicator_Obese1");
                }
                else
                {
                    category  = GetLoc("BMI_Category_Obese2");
                    detail    = GetLoc("BMI_Detail_Obese2");
                    indicator = GetLoc("BMI_Indicator_Obese2");
                }
                ...
                BmiResultText.Text    = string.Format(GetLoc("BMI_Result_Title"), bmi);
                BmiCategoryText.Text  = $"{category}\n{detail}\n\n{string.Format(GetLoc("BMI_Ideal_Format"), minW, maxW)}";
        ...
        private void CalcWeightGoal_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                if (days <= 0) { ShowError(GetLoc("Msg_InvalidInput")); return; }
                ...
                if (gaining)
                {
                    ...
                    {
                        emoji = "✅"; 
                        feasibility = GetLoc("WG_Status_Healthy");
                        advice = GetLoc("WG_Advice_Gain");
                    }
                    else if (kgPerWeek <= 0.5)
                    {
                        emoji = "⚠️"; 
                        feasibility = GetLoc("WG_Status_Hard");
                        advice = string.Format(GetLoc("WG_Advice_Gain") + "\n" + GetLoc("WG_DailyCal_Format"), GetLoc("WG_Surplus"), kcalPerDay);
                    }
                    else
                    {
                        emoji = "🔴"; 
                        feasibility = GetLoc("WG_Status_Dangerous");
                        advice = string.Format(GetLoc("WG_Advice_Gain") + "\n" + GetLoc("WG_DailyCal_Format"), GetLoc("WG_Surplus"), kcalPerDay);
                    }
                }
                else
                {
                    if (kgPerWeek <= 0.5)
                    {
                        emoji = "✅"; 
                        feasibility = GetLoc("WG_Status_Healthy");
                        advice = GetLoc("WG_Advice_Loss");
                    }
                    else if (kgPerWeek <= 1.0)
                    {
                        emoji = "⚠️"; 
                        feasibility = GetLoc("WG_Status_Hard");
                        advice = string.Format(GetLoc("WG_Advice_Loss") + "\n" + GetLoc("WG_DailyCal_Format"), GetLoc("WG_Deficit"), kcalPerDay);
                    }
                    else if (kgPerWeek <= 1.5)
                    {
                        emoji = "🟠"; 
                        feasibility = GetLoc("WG_Status_Expert");
                        advice = string.Format(GetLoc("WG_Advice_Loss") + "\n" + GetLoc("WG_DailyCal_Format"), GetLoc("WG_Deficit"), kcalPerDay);
                    }
                    else
                    {
                ...
                WgDetailText.Text = string.Format(GetLoc("WG_Result_Format"), current, target, gaining ? "+" : "-", diff, days) + "\n" +
                                     string.Format(GetLoc("WG_Weekly_Format"), gaining ? "+" : "-", kgPerWeek) + "\n" +
                                     string.Format(GetLoc("WG_DailyCal_Format"), gaining ? GetLoc("WG_Surplus") : GetLoc("WG_Deficit"), kcalPerDay) + "\n\n" +
                                     advice;
        ...
        private void CalcVTaper_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                string emoji;
                if (shr >= idealSHR + 0.1)
                { emoji = "🏆"; shrRating = GetLoc("VTaper_Excellent"); }
                else if (shr >= idealSHR)
                { emoji = "✅"; shrRating = GetLoc("VTaper_Ideal"); }
                else if (shr >= idealSHR - 0.1)
                { emoji = "⚠️"; shrRating = GetLoc("VTaper_Borderline"); }
                else if (shr >= idealSHR - 0.2)
                { emoji = "🟡"; shrRating = GetLoc("VTaper_NeedsWork"); }
                else
                { emoji = "🔴"; shrRating = GetLoc("VTaper_Far"); }

                string shoulderAdvice = shoulderDiff > 0
                    ? string.Format(GetLoc("VTaper_Shoulder_Expand"), shoulderDiff)
                    : GetLoc("VTaper_Shoulder_Perfect");

                string waistAdvice = waistDiff > 0
                    ? string.Format(GetLoc("VTaper_Waist_Thin"), waistDiff)
                    : GetLoc("VTaper_Waist_Perfect");

                VtResultText.Text = $"{emoji} {shrRating}  (SHR: {shr:F2})";
                VtDetailText.Text = string.Format(GetLoc("VTaper_Detail_Format"), shr, idealSHR, height, idealShoulder, shoulder, idealWaist, waist, shoulderAdvice, waistAdvice);
        ...
        private void CalcWater_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                string rating;
                if (totalL < 1.5)       rating = GetLoc("Water_Rating_Low");
                else if (totalL < 2.5)  rating = GetLoc("Water_Rating_Normal");
                else if (totalL < 3.5)  rating = GetLoc("Water_Rating_High");
                else                    rating = GetLoc("Water_Rating_Athlete");

                string climateNote = WaClimate.SelectedIndex == 0 ? GetLoc("Water_Clim_Temperate") :
                                     WaClimate.SelectedIndex == 1 ? GetLoc("Water_Clim_Hot") : GetLoc("Water_Clim_VeryHot");

                WaResultText.Text = $"💧 {totalL:F1} " + GetLoc("Label_LitersPerDay") + $" ({rating})";
                WaDetailText.Text = string.Format(GetLoc("Water_Detail_Format"), weight, basePerKg, baseWater, actAdd, climateNote, climateMult, totalMl, totalL, glasses, totalMl * 0.35 / 250, totalMl * 0.40 / 250, totalMl * 0.25 / 250);
        ...
        private void CalcSupplement_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (s >= threshold)
                results.Add((GetLoc("Sup_01_Name"), s, GetLoc("Sup_01_Dose"), GetLoc("Sup_01_Time"), GetLoc("Sup_01_Note")));
            ...
            if (s >= threshold)
            {
                ...
                results.Add((GetLoc("Sup_02_Name"), s,
                    string.Format(GetLoc("Sup_02_Dose"), targetP, wt ? "2.0" : "1.6"),
                    GetLoc("Sup_02_Time"), GetLoc("Sup_02_Note")));
            }
            ...
            if (s >= threshold)
                results.Add((GetLoc("Sup_03_Name"), s, GetLoc("Sup_03_Dose"), GetLoc("Sup_03_Time"), GetLoc("Sup_03_Note")));

            // 4. Magnezyum
            s = 0;
            if (chkCantSl) s += 30; if (chkStress) s += 25; if (sleep <= 4) s += 20; if (chkPoorSl) s += 15;
            if (s >= threshold)
                results.Add((GetLoc("Sup_04_Name"), s, GetLoc("Sup_04_Dose"), GetLoc("Sup_04_Time"), GetLoc("Sup_04_Note")));

            // 5. Omega-3
            s = 0;
            if (digestion <= 4) s += 20; if (veggie <= 4) s += 15; if (chkLowE) s += 10; if (wt) s += 15;
            if (s >= threshold)
                results.Add((GetLoc("Sup_05_Name"), s, GetLoc("Sup_05_Dose"), GetLoc("Sup_05_Time"), GetLoc("Sup_05_Note")));

            // 6. Multivitamin
            s = 0;
            if (veggie <= 3) s += 35; if (chkNoVeg) s += 25; if (meal <= 3) s += 15; if (protein <= 3) s += 10;
            if (s >= threshold)
                results.Add((GetLoc("Sup_06_Name"), s, GetLoc("Sup_06_Dose"), GetLoc("Sup_06_Time"), GetLoc("Sup_06_Note")));

            // 7. Kafein
            s = 0;
            if (wt) { if (trainE <= 3) s += 35; if (chkFaint) s += 25; if (chkLowE) s += 15; }
            if (s >= threshold)
                results.Add((GetLoc("Sup_07_Name"), s,
                    string.Format(GetLoc("Sup_07_Dose"), weight * 3, weight * 6),
                    GetLoc("Sup_07_Time"), GetLoc("Sup_07_Note")));

            // 8. Probiyotik
            s = 0;
            if (digestion <= 3) s += 40; else if (digestion <= 5) s += 20; if (meal <= 3) s += 10;
            if (s >= threshold)
                results.Add((GetLoc("Sup_08_Name"), s, GetLoc("Sup_08_Dose"), GetLoc("Sup_08_Time"), GetLoc("Sup_08_Note")));

            // 9. Elektrolit
            s = 0;
            if (chkSweat) s += 40; if (wt && trainE <= 4) s += 15; if (chkFaint) s += 20;
            if (s >= threshold)
                results.Add((GetLoc("Sup_09_Name"), s, GetLoc("Sup_09_Dose"), GetLoc("Sup_09_Time"), GetLoc("Sup_09_Note")));

            // 10. Çinko + C Vitamini
            s = 0;
            if (chkLowE) s += 20; if (veggie <= 4) s += 20; if (chkSweat) s += 15; if (sleep <= 4) s += 10;
            if (s >= threshold)
                results.Add((GetLoc("Sup_10_Name"), s, GetLoc("Sup_10_Dose"), GetLoc("Sup_10_Time"), GetLoc("Sup_10_Note")));
            ...
            if (!isMinimal)
            {
                ...
                if (s >= threshold)
                    results.Add((GetLoc("Sup_11_Name"), s, GetLoc("Sup_11_Dose"), GetLoc("Sup_11_Time"), GetLoc("Sup_11_Note")));
            }
            ...
            if (s >= threshold)
                results.Add((GetLoc("Sup_12_Name"), s, GetLoc("Sup_12_Dose"), GetLoc("Sup_12_Time"), GetLoc("Sup_12_Note")));
            ...
            if (!isMinimal)
            {
                ...
                if (s >= threshold)
                    results.Add((GetLoc("Sup_13_Name"), s, GetLoc("Sup_13_Dose"), GetLoc("Sup_13_Time"), GetLoc("Sup_13_Note")));
            }
            ...
            {
                Text = string.Format(GetLoc("Sup_Analysis_Format"),
                    isMinimal ? GetLoc("Sup_Minimal") : GetLoc("Sup_Comprehensive"),
                    weight,
                    wt ? GetLoc("Sup_Yes") : GetLoc("Sup_No")),
            ...
            else
            {
                ...
                foreach (var r in results)
                {
                    ...
                    if (r.score >= 60) { priorityText = GetLoc("Sup_Priority_High"); priorityColor = "#E74C3C"; }
                    else if (r.score >= 40) { priorityText = GetLoc("Sup_Priority_Medium"); priorityColor = "#F39C12"; }
                    else { priorityText = GetLoc("Sup_Priority_Low"); priorityColor = "#27AE60"; }
